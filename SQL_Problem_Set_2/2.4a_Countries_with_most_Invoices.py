##  In this problem set we'll pretend the Chinook database is the data for your own personal music shop!
##  We'll be marketing your shop and hosting Music Festival using answers we get from 
##  the data stored in your database.
##  Let's get started :)


##  First, you'd like to run a promotion targeting the 3 countries with the highest number of invoices.  
##  Let's find out who they are!
##  Write a query that returns the 3 countries with the highest number of invoices.
##  (Use Test Run to see your resulting dataset compared to the solution)


QUERY = '''SELECT BillingCountry, COUNT(*) AS totalInvoices
FROM Invoice 
GROUP BY BillingCountry 
ORDER BY totalInvoices 
DESC 
LIMIT 3;
'''


'''
---VISUAL GUIDE--- 
(The visual guides are meant to help show you how your queries are interacting with the database)

Before Query...

#################
#   Invoice     #              <----- FROM
################# 
| BillingCountry|  COUNT(*)   <----- SELECT
+===============+

After Query...

################################
#           Invoice            #   <----- RESULT!
################################  
| BillingCountry|   COUNT(*)   |  
+===============+==============+
'''

