...
########################################################################
#ISSUE lat, long in 'ways'
#Program won't write to ways.csv without lat, long here but they are not actually in dataset
#Solution below rewrites a new csv without these columns.  Better solution needed.
    'ways': ('id', 'version', 'timestamp', 'changeset', 'uid', 'user', 'lat','lon'),
...
with open("ways.csv","rb") as source:
    rdr= csv.reader( source )
    with open("way.csv","wb") as result:
        wtr= csv.writer( result )
        for r in rdr:
            wtr.writerow( (r[0], r[1], r[3], r[4], r[5]) )
os.remove('ways.csv')